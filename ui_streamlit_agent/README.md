...
- 💬 Realtime Vanna Agent chat (SSE/WebSocket-compatible UI)
...
The **Agent Chat** page (new in Vanna 2.x) streams responses directly from the backend `/vanna/api/vanna/v2/chat_sse` endpoint. Highlights:

- Uses the same JWT from the login step—no additional credentials required.
- Streams components (rich text, notifications, tables) in near real time.
- Maintains a `conversation_id` so you can restart or continue long-running sessions.
- Exposes a "Start New Conversation" button that clears server-side context.

If you need a WebSocket consumer, the API already exposes `/vanna/api/vanna/v2/chat_websocket`; the Streamlit UI currently favors SSE for compatibility, but the `client.stream_agent_chat` helper makes it easy to experiment with other transports.
